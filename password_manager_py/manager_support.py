...
def add_descriptionExpl():
    """ return the explanation of the add_description command """
    return "\tsyntax: add_description group, value\n" \
           "\tgroup is the group you want to add your note," \
           "value the note you want to write\n\n"
...
def explanationManager(command):
    ...
    if command == "hadd":
        ...
        print(helpExpl())
    elif command == "hadd_description":
        print(add_descriptionExpl())
    else:
        return True
...
def executeCommand(manager, command):
    ...
    if opcode == "add" or opcode == "a":
        ...
        while decryption_key == "first" or decryption_key == "h" or decryption_key == "help":
            decryption_key = input("\n#what decryption key do you want to use?\n"
        ...
        return manager
    # for the commands below I need to check if the group exists
    manager.check_group(addressing_mode[0])
    if opcode == "print_group" or opcode == "pg":
        print(manager.group_data_string(addressing_mode[0]))
        return manager
    elif opcode == "add_description":
        a = " ".join(addressing_mode[1:])
        manager.add_decription(addressing_mode[0], " ".join(addressing_mode[1:]))
        return manager
    elif opcode == "get" or opcode == "g":
        decryption_key = "first"
        while decryption_key == "first" or decryption_key == "h" or decryption_key == "help":
            decryption_key = getpass("\n#what decryption key do you want to use?\n"
                                     ">if you don't know what I'm asking for "
                                     "write 'h' or 'help'\n"
                                     ">if you want to go back write 'b' or 'back'\n")
            if decryption_key == "b" or decryption_key == "back":
                return manager
